...
async function updateTracker(type){
    if(!type){
        ...
        const items = await list.get(type) || []
    ...
    */
    ...
    async cacheCheck(name,apiCall){
        ...
        if(list){
            ...
            age = now - list.timestamp;
            minutes = age / (1000 * 60);
            console.log('Save is '+Math.round(minutes)+" minutes old")
        }
        if(!list || minutes>5){
            if(minutes>5){console.log("updating list...")}
            if(!list){console.log("loading lsit for the first time")}
            const save = {
                data: await apiCall,
...
async function toggleList(id,type,forced){
    ...
    const list = await list.get(type)
    if(!forced){
      forced=0
    }
    let updatedList = []
